...
class CameraWrapper 
{
    public:
        ...
        void Value_Depth_to_Pcl(const cv::Mat& image_cv_depth, 
            pcl::PointCloud<pcl::PointXYZ> &cloud)
        {
            cloud.clear();
            const auto& intrin = camera.get_DepthIntrinsics();
            uint16_t* depth_data = image_cv_depth.ptr<uint16_t>(0);
            for (int v = 0; v < image_cv_depth.rows; v+=9)
            {
                for (int u = 0; u < image_cv_depth.cols; u+=9) 
                {   
                    float depth_value = static_cast<float>(depth_data[v * image_cv_depth.cols + u] * intrin._depth_scale);
                    if(depth_value != 0)
                    {
                        float x = (u - intrin._cx) * depth_value / intrin._fx;
                        float y = (v - intrin._cy) * depth_value / intrin._fy;
                        float z = depth_value;
        ...
        void Value_Mask_to_Pcl(const cv::Mat& image_cv_depth, 
            pcl::PointCloud<pcl::PointXYZ>& cloud, const yolo::BoxArray& objs)
        {
            cloud.clear();
            const auto& intrin = camera.get_DepthIntrinsics();
            uint16_t* depth_data = image_cv_depth.ptr<uint16_t>(0);
            for(auto &obj : objs)
            {
                for (int v = obj.top; v < obj.top + 4 * obj.seg->height; v+=2)
                {
                    ...
                    {     
                        if(u < image_cv_depth.cols && v < image_cv_depth.rows)
                        {
                            float depth_value = static_cast<float>(depth_data[v * image_cv_depth.cols + u] * intrin._depth_scale);
                            if(depth_value != 0)
                            {   
