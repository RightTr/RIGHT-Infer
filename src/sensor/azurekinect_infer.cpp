...
void K4a_Infer::Color_With_Mask(cv::Mat &image_cv_color, yolo::BoxArray &objs)
{
    for (auto &obj : objs) 
    {
...
void K4a_Infer::Depth_With_Mask(cv::Mat &image_cv_depth, yolo::BoxArray &objs)
{
    for (auto &obj : objs) 
    {   
...
void K4a_Infer::Value_Depth_to_Pcl(pcl::PointCloud<pcl::PointXYZ> &cloud)
{
    cloud.clear();
...
void K4a_Infer::Value_Mask_to_Pcl(pcl::PointCloud<pcl::PointXYZ> &cloud, yolo::BoxArray &objs)
{
    cloud.clear();
    uint16_t* depth_data = (uint16_t*)image_k4a_depth_to_color.get_buffer();
    for(auto &obj : objs)
