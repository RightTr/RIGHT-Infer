...
void K4A::K4a_Image_to_Cv(cv::Mat &image_cv_color, cv::Mat &image_cv_depth)
{   
...
void K4A::K4a_Save_Image(int amount)
{   
...
void K4A::K4a_Color_With_Mask(cv::Mat &image_cv_color, yolo::BoxArray objs)
{
    for (auto &obj : objs) 
    {
        uint8_t b, g, r;
...
void K4A::K4a_Depth_With_Mask(cv::Mat &image_cv_depth, yolo::BoxArray objs)
{
    for (auto &obj : objs) 
    {
        uint8_t b, g, r;
        ...
        if (obj.seg) 
        {
            if(obj.left >= 0 && obj.seg->width >=0 && obj.left + obj.seg->width < image_cv_depth.cols && obj.top >= 0 && obj.seg->height >= 0 && obj.top + obj.seg->height <= image_cv_depth.rows)
            {
                ...
                cv::addWeighted(image_cv_depth(cv::Rect(obj.left, obj.top, obj.right - obj.left, obj.bottom - obj.top)), 1.0, mask_depth, 0.8, 0.0, mask_depth);  
...
void K4A::K4a_Mask_to_Binary(cv::Mat &image_cv_binary, yolo::BoxArray objs)
{
