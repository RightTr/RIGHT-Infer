...
int main(int argc, char const *argv[])
{
    ...
    vector<float> left(3), right(3);
    float disparity_center, disparity_left, disparity_right = 0;
    float disparity_ave = 0.;
    float depth = 0;
    ...
    while (1)
    {
        ...
        Pixels_Extract(objs_left, image_infrared_left, left);
        Pixels_Extract(objs_right, image_infrared_right, right);
        
        disparity_center = left[0] - right[0];
        disparity_left = left[1] - right[1];
        disparity_right = left[2] - right[2];
        disparity_ave = (disparity_center + disparity_left * 0.33 + disparity_right * 0.66) / 1.99;
        depth = (fx * 0.059) / disparity_ave; 
        
        target2left_x = (left[0] - cx) * depth / fx;
        target2left_z = depth;
        ...
        cout << "disparity_ave: " << disparity_ave << ", depth: " << depth 
